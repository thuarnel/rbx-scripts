...
Title.TextSize = 18
Title.Text = 'Infinite Yield FE (Rework)'
...
addcmd('fasttp', 'Teleports a player to another player (less reliable) (YOU NEED A TOOL)', {'fastteleport'},function(args, speaker)
	local players1=getPlayer(args[1], speaker)
	local players2=getPlayer(args[2], speaker)
	for i,v in pairs(players1)do
		if getRoot(Players[v].Character) and getRoot(Players[players2[1]].Character) then
...
addcmd('spin', 'Spins your character', {}, function(args, speaker)
	local spinSpeed = 20
	if args[1] and isNumber(args[1]) then
		spinSpeed = args[1]
	end
...
addcmd('unspin', 'Disables spin', {}, function(args, speaker)
	for i,v in pairs(getRoot(speaker.Character):GetChildren()) do
		if v.Name == "Spinning" then
			v:Destroy()
		end
...
addcmd("xray", 'Makes all parts in workspace transparent', {}, function(args, speaker)
    xrayEnabled = true
    xray()
end)

addcmd("unxray", 'Restores transparency', {"noxray"}, function(args, speaker)
    xrayEnabled = false
    xray()
end)

addcmd("togglexray", 'Toggles all part transparency in the workspace', {}, function(args, speaker)
    xrayEnabled = not xrayEnabled
    xray()
end)

addcmd("loopxray", 'Makes all parts in workspace transparent but looped', {}, function(args, speaker)
    if xrayLoop then
        xrayLoop:Disconnect()
    end
...
addcmd("unloopxray", 'Disables loopxray', {}, function(args, speaker)
    if xrayLoop then
        xrayLoop:Disconnect()
    end
end)
...
addcmd('walltp', 'Teleports you above/over any wall you run into', {},function(args, speaker)
	local torso
	if r15(speaker) then
...
addcmd('unwalltp', 'Disables walltp', {'nowalltp'}, function(args, speaker)
	if walltpTouch then
...
autoclicking = false
addcmd('autoclick', 'Turns off autoclick', {},function(args, speaker)
...
addcmd('mousesensitivity', 'Sets your mouse sensitivity (affects first person and right click drag) (default is 1)', {'ms'},function(args, speaker)
	UserInputService.MouseDeltaSensitivity = args[1]
end)

local nameBox = nil
local nbSelection = nil
addcmd('hovername', 'Shows a players username when your mouse is hovered over them', {}, function(args, speaker)
	execCmd('unhovername')
	wait()
...
addcmd('unhovername', 'Turns off hovername', {'nohovername'}, function(args, speaker)
...
addcmd('headsize', 'Expands the head size for players Head (default is 1)', {}, function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
...
addcmd('hitbox', 'Expands the hitbox for players HumanoidRootPart (default is 1)', {}, function(args, speaker)
	local players = getPlayer(args[1], speaker)
...
addcmd('stareat', 'Stare / look at a player', {'stare'}, function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players) do
		if stareLoop then
...
addcmd('unstareat', 'Disables stareat', {'unstare', 'nostare', 'nostareat'}, function(args, speaker)
	if stareLoop then
		stareLoop:Disconnect()
	end
end)
...
addcmd("rolewatch", 'Notify if someone from a watched group joins the server', {}, function(args, speaker)
...
addcmd("rolewatchstop", 'Disable Rolewatch', {}, function(args, speaker)
...
addcmd("rolewatchleave", 'Toggle if you should leave the game if someone from a watched group joins the server', {"unrolewatch"}, function(args, speaker)
...
addcmd("staffwatch", 'Notify if a staff member of the game joins the server', {}, function(args, speaker)
...
addcmd("unstaffwatch", 'Disable Staffwatch', {}, function(args, speaker)
    if staffwatchjoin then
        staffwatchjoin:Disconnect()
    end
...
addcmd('removeterrain', 'Removes all terrain', {'rterrain','noterrain'},function(args, speaker)
	workspace:FindFirstChildOfClass('Terrain'):Clear()
end)

addcmd('clearnilinstances', 'Removes nil instances', {'nonilinstances', 'cni'}, function(args, speaker)
...
addcmd('destroyheight', 'Sets FallenPartsDestroyHeight', {'dh'}, function(args, speaker)
...
addcmd("fakeout", 'Tp to the void and then back (useful to kill people attached to you)', {}, function(args, speaker)
    local root = getRoot(speaker.Character)
...
antivoidloop = false
addcmd("antivoid", 'Prevents you from falling into the void by launching you upwards', {}, function(args, speaker)
...
addcmd("unantivoid", 'Disables antivoid', {"noantivoid"}, function(args, speaker)
    antivoidloop:Disconnect()
    antivoidloop = nil
    if args[1] ~= "nonotify" then notify("antivoid", "Disabled") end
end)

addcmd('trip', 'Makes your character fall over', {}, function(args, speaker)
...
addcmd("removeads", 'Automatically removes ad billboards', {"adblock"}, function(args, speaker)
    while wait() do
        pcall(function()
            for i, v in pairs(workspace:GetDescendants()) do
...
addcmd("scare", 'Teleports in front of a player for half a second', {"spook"}, function(args, speaker)
    local players = getPlayer(args[1], speaker)
    local oldpos = nil
...
addcmd("alignmentkeys", 'Enables the left and right alignment keys (comma and period)', {}, function(args, speaker)
...
addcmd("unalignmentkeys", 'Disables the alignment keys', {"noalignmentkeys"}, function(args, speaker)
...
addcmd("ctrllock", 'Binds Shiftlock to LeftControl', {}, function(args, speaker)
...
addcmd("unctrllock", 'Re-binds Shiftlock to LeftShift', {}, function(args, speaker)
    local mouseLockController = speaker.PlayerScripts:WaitForChild("PlayerModule"):WaitForChild("CameraModule"):WaitForChild("MouseLockController")
    local boundKeys = mouseLockController:FindFirstChild("BoundKeys")
...
addcmd("listento", 'Snoop around someone\'s proximity', {}, function(args, speaker)
    execCmd("unlistento")
    if not args[1] then return end
...
addcmd("unlistento", 'Disables listento', {}, function(args, speaker)
    SoundService:SetListener(Enum.ListenerType.Camera)
    listentoChar:Disconnect()
end)

addcmd("jerk", 'Gives you a tool to jork it', {}, function(args, speaker)
    local humanoid = speaker.Character:FindFirstChildWhichIsA("Humanoid")
...
addcmd("guiscale", 'Changes the size of the gui. [number] accepts both decimals and whole numbers. Min is 0.4 and Max is 2', {}, function(args, speaker)
    if args[1] and isNumber(args[1]) then
...
addcmd("unsuspendvc", 'Unsuspends you from voice chat', {}, function(args, speaker)
...
addcmd('thawunanchored','Thaws unanchored parts',{'thawua','unfreezeunanchored','unfreezeua'},function(args, speaker)
    if freezingua then
...
addcmd('tpunanchored','Teleports unanchored parts to a player',{'tpua'},function(args, speaker)
    local players = getPlayer(args[1], speaker)
    for i,v in pairs(players) do
        local Forces = {}
...
addcmd('autokeypress','Automatically presses a key with a set delay',{'keypress'},function(args, speaker)
...
addcmd('unautokeypress','Stops autokeypress',{'noautokeypress','unkeypress','nokeypress'},function(args, speaker)
	autoKeyPressing = false
	if cancelAutoKeyPress then cancelAutoKeyPress:Disconnect() end
end)

addcmd('addplugin','Add a plugin via command',{'plugin'},function(args, speaker)
	addPlugin(getstring(1))
end)

addcmd('removeplugin','Remove a plugin via command',{'deleteplugin'},function(args, speaker)
	deletePlugin(getstring(1))
end)

addcmd('reloadplugin','Reloads a plugin',{},function(args, speaker)
...
addcmd("addallplugins",'Adds all available plugins from the workspace folder', {"loadallplugins"}, function(args, speaker)
